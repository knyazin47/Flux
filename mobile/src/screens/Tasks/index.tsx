...
import {
  ...
  Modal, ActivityIndicator, StyleSheet, Pressable,
} from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";
import { useTheme } from "@/context/ThemeContext";
import { lsGet, lsGetAsync, lsSet, lsRemove, incrementTodayDone, addXP, updateTopicStats, checkAchievements } from "@/utils/storage";
...
function OrangeBtn({ label, onPress, outlined }: { label: string; onPress: () => void; outlined?: boolean }) {
  return (
    <TouchableOpacity activeOpacity={0.85} onPress={onPress}
      style={outlined ? s.btnOutlined : s.btn}>
...
export default function TasksScreen({ navigation }: TasksHomeProps) {
  ...
  const saveSnapshot = useCallback((overrides: Partial<SessionData> = {}) => {
    const snap: SessionData = { questions, qIndex, answers, timerDuration, activeTopic, currentSelected: selected, ...overrides };
    lsSet("tasks_session", snap); // fire-and-forget
  ...
  const handleExit = () => {
    ...
    setSelected(null); setView("start"); setConfirmExit(false);
  };
  ...
  const startSession = async () => {
    ...
    try {
      const topic = activeTopic === "Все"
        ...
        : activeTopic;
      const qs = await generateQuestions(topic, SESSION_COUNT, 2) as Question[];
  ...
  const handleNext = useCallback((timeout = false) => {
    ...
    const proceed = () => {
      if (isLast) {
        ...
        addXP(correct * 10);
        incrementTodayDone(questions.length);
        ...
        checkAchievements();
        setAnswers(newAnswers); setView("results");
      } else {
        setAnswers(newAnswers); setQIndex(nextIdx);
      }
    };

    if (timeout) { proceed(); }
    else { setSelected(null); setTimeout(proceed, 300); }
  }, [questions, qIndex, selected, answers, saveSnapshot]);
    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: theme.bg }}>
        ...
        <ScrollView contentContainerStyle={{ padding: 16, gap: 12, paddingBottom: 96 }}>
          ...
          {/* Options — FlatList for performance */}
          <FlatList
            data={q.options}
